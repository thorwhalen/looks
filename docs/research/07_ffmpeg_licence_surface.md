...
Every claim was re-checked independently — `configure`/`LICENSE.md`/`allfilters.c`/`Makefile` re-fetched at n8.1, every licence text re-fetched from its origin, every command re-run on this machine, and the note's probe code extracted and executed. **One thing the note could not do, this review did: FFmpeg n8.1 was configured and built `--disable-gpl` in-session**, producing a real `LGPL version 2.1 or later` binary to measure against. Findings below; the author's text above is unchanged.

### Refuted

**R1 — `libx264`/`libx265` are NOT "the only software H.264/HEVC encoders FFmpeg has" (§4, and recommendation 5 rests on it).** At n8.1 `configure` also wires `libopenh264_encoder_deps="libopenh264"` (line 3800) and `libkvazaar_encoder_deps="libkvazaar"` (line 3785). Neither library is in `EXTERNAL_LIBRARY_GPL_LIST`; both sit in the plain `EXTERNAL_LIBRARY_LIST`. OpenH264 is **BSD-2-Clause** (Cisco, `LICENSE`), Kvazaar is **BSD-3-Clause** (Tampere University, `LICENSE`). This is not theoretical: the `imageio-ffmpeg` 7.1 binary already on this laptop is built `--enable-libkvazaar`, lists `V....D libkvazaar libkvazaar H.265 / HEVC (codec hevc)`, and encoded 10 frames of `testsrc2` at 320×192 to a 33 307-byte HEVC mp4. So **"an LGPL-tier render cannot output software-encoded H.264/HEVC anywhere, ever" is false.** The note's own §7 lists `libopenh264` and `libkvazaar` among unverified externals, so the absolute claim contradicts its own text. Corrected statement: FFmpeg has no *in-tree* H.264/HEVC encoder, and the two *ubiquitous* external ones are GPL — but BSD-licensed external H.264 (OpenH264) and HEVC (Kvazaar) encoders exist, are LGPL-build-compatible, and are shipped by at least one binary in this environment. The practical constraint survives ("most builds you meet won't have them, and the patent perimeter is unchanged"); the absolute one does not.

**R2 — `lensfun` is NOT the only filter where `configure` and the source headers disagree.** Two more, and this time the leak reaches a *default* LGPL build. `libavfilter/perlin.c` and `libavfilter/qp_table.c` both carry `GNU General Public License … version 2 of the License, or (at your option) any later version` headers, and **neither appears in `LICENSE.md`**. Neither the `codecview` filter nor the `perlin` filter has any `_filter_deps` line in `configure` — no gpl gate at all — while `libavfilter/Makefile` says, unconditionally:

```
237:OBJS-$(CONFIG_CODECVIEW_FILTER) += vf_codecview.o qp_table.o
627:OBJS-$(CONFIG_PERLIN_FILTER)    += vsrc_perlin.o perlin.o
```

Measured on the in-session `--disable-gpl --disable-version3 --disable-nonfree` build: `License: LGPL version 2.1 or later`, `CONFIG_GPL 0`, yet `CONFIG_CODECVIEW_FILTER=yes`, `CONFIG_PERLIN_FILTER=yes`, both `&ff_vf_codecview` and `&ff_vsrc_perlin` present in the generated `libavfilter/filter_list.c`, `ffmpeg -filters` lists both, and `make -n libavfilter/qp_table.o` / `libavfilter/perlin.o` each emits a real `gcc … -c` line. **Two GPL-2-headed translation units are compiled into a binary whose `-L` says LGPL-2.1.** The note's `filter_tier()` returns `LGPL_2_1` for both and `require()` **ALLOWED** them at `max_effect_tier=LGPL_2_1` against that binary — verified by running the note's own code. Add `codecview` and `perlin` to the hardcode-around list alongside `lensfun`.
*Checked and cleared:* `removegrain`'s GPL x86 asm is a false alarm — `libavfilter/x86/Makefile:49` wraps it in `ifdef CONFIG_GPL`, so FFmpeg handles that one correctly.

**R3 — axis (b) of the three-axis reconciliation does not return 33.** Scanning all 534 `libavfilter/*.c` at n8.1 (from the tag tarball) for a `GNU General Public License` header without `Lesser` returns **37**: the 33, plus `perlin.c`, `phase_template.c`, `qp_table.c`, `vf_fsppdsp.c`. The note accounts only for `vf_fsppdsp.c`; two of the other three are the leak in R2. The reconciliation as written also omits that `vf_lensfun.c` is the entry which brings `LICENSE.md`'s libavfilter list to 33 — so there are four bookkeeping facts, not three, and the two 33s are different sets that happen to have the same cardinality. **Axes (a) and (c) are confirmed exactly** (my own grep returns the same 33 names; `LICENSE.md` lists the same 33 files); only the header-scan figure is wrong.

**R4 — "Not one of them is a colour operation" (verdict) is false.** From `ffmpeg -filters` on the local binary: `eq V->V Adjust brightness, contrast, gamma, and saturation.`, `histeq V->V Apply global color histogram equalization.`, `colormatrix V->V Convert color matrix.` Three GPL-gated colour operations, two of which the note's own §3 lists as GPL. The defensible claim is the weaker one made later: *every* GPL-gated colour operation has an LGPL substitute. Say that instead — in a refusal engine's headline, the strong form is the kind of sentence that gets quoted back later.

**R5 — claim 13's stated mechanism is wrong (the conclusion is right, and is now measured).** `die_license_disabled` is mapped **only** over `EXTERNAL_LIBRARY_*_LIST` (configure 4767-4771); filters are not in those lists and are *silently* disabled by ordinary dependency resolution, then omitted from the configure-generated `libavfilter/filter_list.c` (`print_enabled_components`, line 8803), which is what `av_filter_iterate` walks. That is the real mechanism, and it is stronger evidence than the two documents the note cited. **Claim 13 is no longer unverified**: on the in-session LGPL build, `eq`, `boxblur`, `colormatrix`, `histeq`, `cropdetect`, `perspective`, `hqdn3d` and `lensfun` are all absent from `-filters` while `geq`, `lut3d` and `lutrgb` are present, and the Que Calor chain `lut3d=file=identity.cube,lutrgb=…` runs on it, exit 0. Recommendation 7's CI job is still worth having as a regression gate, but the fact itself is established.

**R6 — arithmetic.** "34 names in all, out of 481 filters in the local build (6.7%)" — 34/481 is **7.07%**. 6.7% is 32/481, the number of table entries actually present in the Homebrew build (§5.7 uses it correctly).

**R7 — `GPL_EXTERNAL_LIB_FILTERS` is incomplete as written.** `rubberband_filter_deps="librubberband"` and `librubberband ∈ EXTERNAL_LIBRARY_GPL_LIST`. Harmless for a video package, but the dict's docstring claims to be *the* set of filters gated by a GPL external library.

### Confirmed (independently re-run, not taken on trust)

- **33** `_filter_(deps|select|suggest)` lines containing `gpl` at n8.1, name-for-name identical to the note's list; `LICENSE.md` lists **33** libavfilter files; **534** `.c` files; `vf_fsppdsp.c` exists.
- **`geq` is LGPL-2.1-or-later.** No `geq` token anywhere in `configure@n8.1`; header confirmed; `d5e7f01090a0b738e3df4323910875151f7dbb94`, 2019-12-16, *"avfilter/vf_geq: Relicense to LGPL"*, touching `LICENSE.md`/`configure`/`vf_geq.c`; `compare/n4.2...` = diverged, `compare/n4.3...` = behind → **n4.3, 2020-06-15**.
- **The Que Calor posterise chain is entirely LGPL-2.1+** — `vf_lut3d.c` and `vf_lut.c` headers confirmed, neither name in any gpl deps line, and the chain ran on the GPL-3 Homebrew build **and on the in-session LGPL-2.1 build**.
- **`show_license()`** read at n8.1: exactly the five-way `#if CONFIG_NONFREE / GPLV3 / GPL / LGPLV3 / else` cascade quoted, matching `configure`'s cascade at 4767-4786. OpenSSL rule confirmed at line 7493. Local Homebrew 8.1 is `--enable-gpl --enable-version3 --enable-openssl` and `-L` says GPL v3+.
- **`lensfun`**: `lensfun_filter_deps="liblensfun version3"` (line 4152), no `gpl`; `LICENSE.md:40` *"(GPL version 3 or later)"*; header GPLv3. Also confirmed: `lensfun` is the **only** filter gated on any `EXTERNAL_LIBRARY_VERSION3_LIST` member.
- **`pp` / libpostproc**: `libpostproc` has 11 entries at n7.1 and 404s at n8.0 and n8.1; `ff_vf_pp` absent from `allfilters.c@n8.1`; diffing the gpl-gated name sets gives 34 (n7.1) vs 33 (n8.1) with **`pp` as the only difference**.
- **The `-filters` column widths**, hexdumped on both binaries: 7.1 emits a `..C = Command support` legend row and 3-char flags (`TSC`, `...`), 8.1 drops it, adds a `  ------` separator and emits 2-char flags (`TS`, `..`). The note's arity-column parser was diffed against an independent `awk`-derived ground truth on **three** binaries — 481, 484 and 443 filters — with **zero missing and zero extra** on each. Recommendation 3 is sound.
- **`ffmpeg -hide_banner -h filter=nosuchfilter`** prints `Unknown filter 'nosuchfilter'.` and exits **0**.
- **`imageio-ffmpeg` 0.6.0** bundles `ffmpeg-macos-aarch64-v7.1`, `-L` = GPL v2+, `--enable-gpl --enable-libx264 --enable-libx265 --enable-libvidstab --enable-postproc --enable-libzimg --enable-libkvazaar --enable-libaom`.
- **Nonfree lists** verbatim as quoted; **`lgpl_gpl`** has 11 users (9 audio decoders, 1 parser, 1 bsf) and no video filter.
- **x264 `COPYING`** and **x265 `COPYING`** are both the GPLv2 text; **libvpx** `LICENSE` BSD-3 + `PATENTS` "Additional IP Rights Grant". All nine encoder smokes reproduce on the local binary.
- **The probe code imports, runs, and reproduces §5.6 exactly**, and — the test the note could not run — classifies the **real** LGPL-2.1 binary correctly (`spdx=LGPL-2.1-or-later`, 443 filters), not merely a shell stub.

### Open items in §7, now closed

- **`libzimg` (needed by `zscale`) is WTFPL-2.0** — `COPYING` is *"DO WHAT THE FUCK YOU WANT TO PUBLIC LICENSE, Version 2"*, and its bundled `graphengine` submodule is WTFPL too. Free and GPL-compatible, but **not** in the MIT/BSD/Apache/ISC set this ecosystem's standing policy names, and it carries **no warranty disclaimer and no patent grant**. `zscale` is legally available at the LGPL tier; registering it should be a *recorded* policy decision, not a silent one.
- **`libplacebo` is LGPL-2.1** (`LICENSE` = GNU Lesser General Public License, Version 2.1).
- **The `burns → moviepy → imageio-ffmpeg` edge is verified.** `burns/pyproject.toml` declares `moviepy`; installed `moviepy` **2.2.1** `METADATA` carries `Requires-Dist: imageio_ffmpeg>=0.2.0` as a hard dependency, not an extra. **Bonus, and it matters:** `imageio_ffmpeg` 0.6.0's own `METADATA` says `License: BSD-2-Clause` while the wheel ships a GPL-2.0-or-later executable — so a metadata-based licence scan reports the environment clean. Same shape as the `av` wheel. **The forbidden-deps rule cannot be enforced by reading package metadata.**

### Design objections

1. **`filter_tier()` fails OPEN, against the non-negotiable that unknown is a refusal.** Any name outside the three hardcoded tables returns `Tier.LGPL_2_1`, which the default ceiling allows. Demonstrated above with `codecview`/`perlin` on a real LGPL binary; it also means every filter added after the n8.1 anchor inherits LGPL_2_1 for free, which is the *false-permission* direction. The tier must come from `looks`'s own **registry of registered effects**, each carrying a recorded tier and its evidence line; an unregistered filter name is `UNKNOWN` and refuses. Relatedly, `TABLE_ANCHOR` is a decorative string the code never reads — compare it to `probe.version` and degrade to `UNKNOWN` outside the anchored range.
2. **Recommendation 5 rests on refuted R1, and it pulls execution back into scope.** An encoder licence table plus "an LGPL-tier render cannot output X" is render-time policy, and owning it is one convenience API away from the `looks.render()` the kickoff forbids. A `Look` may *declare* what its tier implies for delivery; the caller's renderer (muvid's `assemble.py`, which owns the bounded-memory invariant) enforces it.
3. **Recommendation 9 pushes the environment into the pure-data spec.** Folding `(path, version, spdx, filter-set digest)` into the *plan* key makes planning require a binary, contradicting recommendation 1's own "decidable with no binary present" and the kickoff's "inspectable, persistable, diffable and costable before anything runs". `falaw.CallPlan` carries `tool`/`application`/`arguments`/`output_kind`/`estimated_cost_usd`/`cache_status` and **no** environment fingerprint. The probe belongs to the compiled command and its cache entry, not to the `Look`.
4. **Rule 4 is drawn one notch too wide.** "Availability comes from `-filters` and nothing else" forbids the only free way to check that a filter's *options* exist. Measured on this laptop: `scale` in 8.1 has `in_primaries`, `in_transfer`, `out_primaries`, `out_transfer`, `reset_sar`; the 7.1 binary's `scale` has none of them. A `Look` compiling `scale=out_primaries=bt709` passes the presence gate on 7.1 and then dies at ffmpeg-exit — the exact failure recommendation 5 exists to prevent, and squarely in colour-management territory. Narrow the rule to "never use `-h filter=`'s **exit code**"; its stdout is a legitimate free option-level probe.
5. **`Tier.PERMISSIVE` is unreachable on both sides** — `filter_tier()` never returns it and `_parse_licence()` cannot. A caller expressing the house policy (MIT/BSD/Apache/ISC only) gets a blanket refusal. The "source licence (file header)" column also invites a category error: invoking `gblur` through an LGPL binary is not a BSD-2 act — the boundary licence is the *binary's*, and the file header is provenance, not tier.
6. **The two-binary hazard is diagnosed but its rule is never written.** §5.7 should end with the corollary the non-negotiables imply: `looks` resolves ffmpeg from `PATH` or an explicit path and **never** falls back to `imageio_ffmpeg.get_ffmpeg_exe()` or any bundled binary — which is what most PyPI ffmpeg wrappers do, and which would silently reintroduce the GPL-2 executable.
7. **Recommendation 8 is stated but not implemented** — `require()` still refuses `NONFREE` by ordinal, not before the ceiling comparison.
8. **No doctests** on any public function (`doctest.testmod` → `attempted=0`), against the stated house style. And the flagship effect is a 3D LUT: the note never asks whether a shipped or third-party `.cube` file carries its own licence tier — a real gap for a registry whose first entry is a gradient-map LUT.

**Bottom line:** the mechanism, the probe rules and the parser are right and are now measured on three binaries including a purpose-built LGPL one. The two things to fix before any of this becomes code are the **fail-open default in `filter_tier()`** and the **encoder claim in §4/recommendation 5**.
