...
| [`08_opencv_and_python_deps.md`](08_opencv_and_python_deps.md) | sound-with-corrections | The optional-extras surface. `opencv-python`'s macOS **arm64** wheels bundle a GPL-3 FFmpeg and the manylinux ones are clean; macOS **x86_64 is version-dependent**, not uniformly clean — see `thorwhalen/looks#21` (4.12.0.88 bundles the same GPL ffmpeg as arm64, 5.0.0.93 has none). |
