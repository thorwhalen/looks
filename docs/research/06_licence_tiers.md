...
An independent reviewer re-ran every measurement in this note on the same machine (macOS arm64, `python` = `~/.pyenv/versions/p12/bin/python` 3.12.12, Homebrew `ffmpeg` 8.1_1). Nothing below was taken on trust: each command was executed, each URL refetched, each wheel opened on disk.

### Confirmed, independently

- **The ffmpeg build.** `ffmpeg -version` → `8.1`, `--prefix=/opt/homebrew/Cellar/ffmpeg/8.1_1 … --enable-version3 … --enable-gpl …`; `ffmpeg -L` → "GNU General Public License … version 3, or (at your option) any later version". libavfilter 11.14.100.
- **The 34/32 filter diff, exactly.** Re-derived with `LC_ALL=C comm` over `ffmpeg -filters` (481 entries) vs `av.filter.filters_available` (447, libavfilter 11.4.100): 34 in the GPL build only, and the set is character-for-character the list in §1.1 plus `libvmaf` and `premultiply_dynamic`. (The author's first `comm` would have shown a spurious `scale_vt` on both sides — locale collation. The published list is right.)
- **The 33 `gpl`-guarded filter declarations** in `release/8.1` `configure`, `eq_filter_deps="gpl"` among them; `EXTERNAL_LIBRARY_GPL_LIST` matches `FFMPEG_GPL_COMPONENTS` exactly; `die_license_disabled` at line 4759 and the licence if/elif chain at 4776–4785, verbatim as quoted.
- **Stability bonus the note under-claims.** §8 warns "the 32-filter figure will drift". It has not: the same 33 declarations and the same `EXTERNAL_LIBRARY_GPL_LIST` appear in `release/9.0` and in `master`. `brew` already offers ffmpeg 9.0.1, so this is checked across the next major.
- **The Que Calor chain is LGPL-clean**: `lut3d`, `lutrgb`, `curves`, `colorchannelmixer`, `colorlevels`, `scale`, `format` all present in av's LGPL libavfilter; `eq` absent. (Also useful for the geometry tier: `gblur`/`avgblur`/`pad`/`crop` are LGPL, `boxblur`/`smartblur`/`sab` are not.)
- **`av` 16.0.1's four-layer disagreement, in full.** `License-Expression: BSD-3-Clause`; `avcodec_license()` → `LGPL version 3 or later`; `avcodec_configuration()` → `--enable-version3 … --enable-libx264 --enable-libx265` with no `--enable-gpl`; `otool -L` → `@loader_path/libx264.165.dylib` (1,691,168 B) and `libx265.215.dylib` (6,397,088 B); `av.codec.Codec('libx264'|'libx265','w')` both construct.
- **`opencv-contrib-python` 4.13.0.92**: metadata `License: Apache 2.0`, in-wheel `LICENSE.txt` MIT "Copyright (c) Olli-Pekka Heinisuo", 93 dylibs, bundled `avutil_license()` → `GPL version 3 or later`, configuration `--prefix=/opt/homebrew/Cellar/ffmpeg/7.1.1_3 … --enable-gpl --enable-version3 … --enable-libx264 --enable-libx265 --enable-libvidstab --enable-librubberband --enable-frei0r`. `opencv-python` 4.12.0.88 ships `libx264.164`/`libx265.215`/`libavcodec`/`libavformat`/`libbluray`/`libaribb24`.
- **`imageio-ffmpeg` 0.6.0**: `BSD-2-Clause` metadata, `binaries/ffmpeg-macos-aarch64-v7.1` at 49.368728 MB, `--enable-gpl --enable-libx264 --enable-libx265`, `-L` prints GPL v2 or later. **`burns` 0.0.9 → `moviepy` 2.2.1 → `imageio_ffmpeg>=0.2.0`**, all three hops read from installed metadata. **`ultralytics` 8.4.75** ships the full AGPLv3 text.
- **AnimeGANv2 and White-box Cartoonization** README licence sections quoted verbatim from the raw files. **SPDX 3.28.0 / 2026-02-20** and **REUSE 3.3 / 2024-11-14** from the live pages. **frei0r** `f0r_plugin_info_t` has exactly the nine fields listed and `COPYING` is GPL-2.0. Every SPDX id in `REACH_BY_SPDX` is a listed, non-deprecated identifier.
- **§7 runs.** Extracted to a file and executed on a bare stdlib interpreter: imports clean, **19 doctests, 19 passed**. `probe_ffmpeg()` on this machine → `GPL-3.0-or-later` / `COPYLEFT_TOOL` / `ADMITTED`; a `WEAK_COPYLEFT` ceiling raises `LicenceCeilingExceeded`; av's real configuration string → `None`.
- **`mixing/video/video_util.py` line 8 is `from moviepy import …` at module scope.** The licence event is real.
- **References 3 and 14 check out** (reelee-web's deliberate `BSD-4-Clause` exclusion; `illustration`'s no-dropped-restriction-token invariant).

### Refuted

1. **`opencv-python-headless` is NOT "not installed here", and it is NOT an escape.** `opencv-python-headless` **4.13.0.92** is installed in the very interpreter every other measurement in this note came from (`av` 16.0.1, `opencv-contrib-python` 4.13.0.92, `imageio-ffmpeg` 0.6.0, `ultralytics` 8.4.75, `moviepy` 2.2.1, `burns` 0.0.9 all match) — and its `dist-info` is dated **2026-06-30**, two months before this note. Its `RECORD` lists `cv2/.dylibs/libx264.164.dylib`, `libx265.215.dylib`, `libavcodec.61.19.101`, `libavfilter`, `libpostproc`, `librubberband`, `libvidstab`, `libaribb24`. **Headless bundles the same GPL ffmpeg.** The note's single most consequential open question was answerable in thirty seconds and the answer closes the "cheap way out".
2. **The in-process linkage is not a reading — it is observable at runtime, unconditionally.** `import cv2` alone (no `VideoCapture`, no `VideoWriter`) maps `libavcodec.61.19.101`, `libavfilter`, `libavformat`, `libavutil`, `libpostproc`, `librubberband`, `libvidstab`, `libx264.164` and `libx265.215` into the live Python process (`vmmap` on the running PID; `cv2.getBuildInformation()` → `FFMPEG: YES`). So "do the flatten in numpy" does **not** avoid it if anything in the process imports `cv2`, and the escalation in §1.3/§13 should say the load is unconditional and at import.
3. **The probe does not implement "FFmpeg's own rule verbatim".** §1.2 quotes *two* `map "die_license_disabled …"` lines; §7 implements half of one. Executed: `ffmpeg_spdx_from_configuration("--enable-libaribb24")` → `LGPL-2.1-or-later`; `"--enable-gmp"` → `LGPL-2.1-or-later`; `"--enable-libsmbclient"` → `LGPL-2.1-or-later`. All three are builds unpatched `configure` cannot produce (`EXTERNAL_LIBRARY_VERSION3_LIST` and `EXTERNAL_LIBRARY_GPLV3_LIST` are guarded by the same mechanism), and all three are silently given the *most permissive* answer. This is the false-permission direction. Fix: two more frozensets and two more contradiction branches.
4. **The shipped ledger rows fail the note's own round-trip test.** §5 says "every row's `tier`/`verdict` must equal `classify(row)`". Executed against the JSON as written: the `opencv` row (`spdx="Apache-2.0 AND GPL-3.0-or-later"`, `reach="none"`, `coupling=in_process`, `conveyance=conveys`) classifies to **`Tier.PERMISSIVE`**, not the declared `copyleft_shipped`. So does the `moviepy/pypi:burns` row. `reach_of("Apache-2.0 AND GPL-3.0-or-later")` is `Reach.UNKNOWN` — compound expressions are not parsed, as documented, but then the explicit `reach` field silently makes the row the *most* permissive of the three available readings, while the prose says "refuse until a human rules". The root cause is structural: a `(provider_id, realisation)` key cannot hold the two components `assess()` requires.
5. **A `nonfree` build is misclassified as unclassifiable.** `ffmpeg_spdx_from_configuration("--enable-nonfree --enable-libfdk_aac")` → `None` → `LicenceUnknown`, whose remedy text is "supply evidence, or select one that can be probed". FFmpeg calls that build "nonfree and unredistributable" — the *best-characterised* refusal there is. Folding known-worst into unknown is exactly what §6 rule Two forbids.
6. **"Porting the geometry tier off moviepy is small work" is not supported by the source.** `resize_to_dimensions` is not arithmetic over `(width, height)`: it calls `video.resized(...)`, builds an `ImageClip` from `np.full(...)`, composites with `CompositeVideoClip`, and the `fill` path runs a **per-frame PIL `GaussianBlur`** through `image_transform` plus `vfx.MultiplyColor`. `numpy` is a second module-scope non-stdlib import the note does not mention. Honest split: a small pure geometry core (target rect / scale factor / `SOCIAL_SIZES`) that ports trivially, and a pixel-work half that must become an ffmpeg filter spec or stay in `mixing`. (Note `boxblur` is GPL-only and `gblur` is not — the ported blur-fill lands straight on this note's own machinery.)
7. **Minor factual slip:** the §1.1 table's "Video filters" column reports 481 / 447. Those are *total* filter counts; this build has 331 video-touching filters. The 32 in the diff are all video filters, so nothing downstream changes.
8. **OpenFX does carry licence-shaped properties**, contra §4's "no `kOfx*` property for licence or copyright": `kOfxStatUnlicensed`, `kOfxImageEffectPropBehaviourWhenUnlicensed`, `kOfxUnlicensedContinue`, `kOfxUnlicensedFail`. They model *entitlement* (watermark-or-fail), not copyright terms — so the substance of the gap claim survives — but the prior-art table's "Does it refuse? **no**" is wrong for OpenFX: it has an explicit host-side refuse-vs-continue knob for the licensing it does model, which is closer prior art for looks' refusal-not-warning rule than the table admits.

### Design defects found by executing §7

- **Opting into a field-of-use restriction silently waives the copyleft ceiling.** `check()` returns early on `FIELD_RESTRICTED` once the `FieldOfUse` is in `allow_field_restricted`, and `classify()` set `tier=None` for that branch, so the ceiling is never applied. Demonstrated: a `SUBPROCESS` + `GPL-3.0-or-later` + `CONVEYS` + `NON_COMMERCIAL` provider is **admitted** under `Policy(max_tier=Tier.PURE, allow_field_restricted={NON_COMMERCIAL})`. This is the exact mirror of the flaw §2.1 exists to prevent, in the false-permission direction. Fix: classify must carry the ladder tier *alongside* the field verdict, and `check()` must fall through to the ceiling test after the opt-in.
- **`assess()` hardcodes `DEFAULT_ORDER`** and takes no `Policy`, so `Policy(order=…)` — the note's headline "replaceable policy projection" — is ignored whenever a provider has more than one component. `_VERDICT_SEVERITY` also ranks `Verdict.OVER_CEILING`, which `classify()` can never return.
- **`classify()` maps `IN_PROCESS` + `LIBRARY` + `CONVEYS` → `COPYLEFT_SHIPPED`**, above the default ceiling. Conveying an LGPL library therefore ranks with conveying a GPL program. Defensible, but this case appears nowhere in the §2.3 ladder table, where rung 2 is described without reference to conveyance.
- **`probe_ffmpeg` crashes rather than refusing** when the binary is absent and `realisation != "system"`: `probe_ffmpeg("/nonexistent/ffmpeg", realisation="pypi:x")` raises `FileNotFoundError`, contradicting its own docstring. `_run` has **no `timeout`**, so §3.3's "a probe that times out" is unhandled — the failure mode is a hang, not a refusal.
- **`Conveyance` is not well defined.** Its docstring says "whether *we* redistribute", but `probe_ffmpeg` derives it from `realisation` alone, so a caller's pre-existing `imageio-ffmpeg` — which `looks` does not declare and does not convey — is recorded `CONVEYS`. The axis that separates rungs 3 and 4 needs to say *whose* closure.

### Unaddressed by the note

- **A `Look` is meant to be persistable and portable (kickoff non-negotiable #2), but gate 3 resolves the tier from the *compiling* machine's ffmpeg.** Nothing says whether the `Assessment` travels with the serialized plan, what happens when a plan compiled on an LGPL machine is executed on a GPL one, or how a cached probe is keyed (exe path? mtime? version string?) so an `ffmpeg` upgrade invalidates it.
- **`looks` never executes, so every `Coupling` it records is a prediction about a downstream consumer it does not control.** §3's proudest point ("`looks` never chooses `libx264` and never has to answer for it") sits awkwardly against an axis describing someone else's linkage. The tier `looks` reports is advice to the executor, not a fact about `looks`; that should be said.
- **`max_tier` is not the "commercial-safe only" knob the kickoff advertises.** Every rung including `COPYLEFT_SHIPPED` is commercially usable; what is not is `FieldOfUse.NON_COMMERCIAL`, which this design correctly puts *off* the ladder. So "commercial-safe only" is `allow_field_restricted=frozenset()` — already the default — and the reconciliation with the kickoff's wording is never made.
- **Recommendation 10's extras test, applied to the current `pyproject.toml`,** demands ledger rows for `pytest`, `pytest-cov` and `ruff` (`dependencies = []`, one `dev` extra). Either dev extras are excluded — a rule the note does not state — or the ledger fills with build-tool rows.
- **Gate 1 is an import-time `ValueError` validated against a JSON file**, so `import looks` does file I/O plus a JSON parse, and a third-party effect registering an unknown provider makes the *importing* package unimportable rather than the effect unusable.
- **The flagship look is refused by its own design.** Que Calor stage one is `cv2.pyrMeanShiftFiltering`, and by this note's own findings that is `COPYLEFT_SHIPPED` (above the default ceiling) at best and `FORBIDDEN` at worst — with headless now ruled out as the escape (refutation 1) and the ffmpeg libraries loading unconditionally at `import cv2` (refutation 2). A resolution path is owed, and the alternatives the note never priced are: an ffmpeg-free OpenCV built from source (`-DWITH_FFMPEG=OFF`), a non-OpenCV mean-shift, or an ffmpeg-native flattener.

**Overall:** the empirical core is unusually solid — every load-bearing measurement reproduced exactly, and one (the GPL-filter set) is more stable across FFmpeg versions than the note claims. The taxonomy's central argument (field-of-use is off the ladder; in-process strong copyleft is not a rung) survives. What does not survive contact is the *implementation* of that argument: the field-of-use opt-in waives the ceiling, the contradiction detector implements half the rule it quotes, the seeded ledger contradicts its own derivation test, and the one open question the note flagged as decisive was already answerable on this machine and answers the wrong way.
