...
| `opencv-python` (non-headless) bundled ffmpeg | `pypi:opencv-python==4.12.0.88, macosx_x86_64` | **same GPL ffmpeg as arm64** (`libx264`/`libx265`, `--enable-gpl --enable-version3`) — issue #21: this platform is **not** uniformly FFmpeg-free | `strings` on the shipped `libavcodec`/`libavutil` | `COPYLEFT_SHIPPED` (GPL-3) |
| `opencv-python` (non-headless) bundled ffmpeg | `pypi:opencv-python==5.0.0.93, macosx_x86_64` | **no FFmpeg at all**, same permissive dylib set as the headless x86_64 wheel — the FFmpeg-free build reached non-headless only by this version | wheel inspection | `PERMISSIVE` |
...
**Unverified rows** (do not cite as fact): opencv-python's macOS x86_64 wheel contents for any version strictly between the two measured endpoints, 4.12.0.88 (GPL ffmpeg) and 5.0.0.93 (none) — where the wheel actually stopped bundling FFmpeg on that platform is not pinned; the Windows opencv wheel contents beyond what note 08's review inspected; `libzimg` and `libplacebo`'s own LICENSE files (both LGPL-2.1+ as FFmpeg *wrapper* code, and absent from every gate list, which is FFmpeg's assertion rather than a verification); torchvision keypoint R-CNN **weights** provenance; whether a source build with `-DWITH_FFMPEG=OFF` actually yields an FFmpeg-free `cv2`; whether conda-forge's `py-opencv` links an LGPL FFmpeg on macOS.
